        ...
        if current_user.is_authenticated:
            try:
                ...
                base_filter = (
                    ...
                    Client.due_date <= due_limit,
                )
                #Uma única query com contagens agregadas
                row = db.session.query(
                    func.count(case((Client.due_date < today, 1))).label("overdue"),
                    func.count(case((Client.due_date == today, 1))).label("today"),
                    func.count(case(
                        (db.and_(Client.due_date > today, Client.due_date <= due_limit), 1)
                    )).label("soon"),
                ...
                alerts = (
                    Client.query.filter(*base_filter)
