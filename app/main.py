...
from app.models import AccountSlot, Attendance, Charge, Client, DirectMessage, Platform, PlatformAccount, TeamMessage, TimeEntry, User
...
def support_delete(attendance_id: int):
    ...
    return redirect(next_url or request.referrer or url_for("main.support_center"))
...
def support_center():
    ...
    if request.method == "POST":
        create_support_entry(selected_client)
        redirect_args = {"client": selected_client.id} if selected_client else {}
        return redirect(url_for("main.support_center", **redirect_args))

    query = Attendance.query.outerjoin(Client)

    if selected_client:
        query = query.filter(Attendance.client_id == selected_client.id)
...
def acessos_list():
    ...
    platforms = Platform.query.order_by(Platform.name.asc()).all()
    clients = Client.query.filter_by(status="ativo").order_by(Client.name.asc()).all()
    return render_template("acessos/list.html", platforms=platforms, clients=clients)


@bp.route("/acessos/plataforma/nova", methods=["GET", "POST"])
@roles_required("admin")
def platform_create():
    from app.models import Platform
    if request.method == "POST":
        ...
        notes = request.form.get("notes", "").strip() or None
        if not name:
            flash("Informe o nome da plataforma.", "danger")
        elif Platform.query.filter_by(name=name).first():
            flash("Ja existe uma plataforma com esse nome.", "danger")
        else:
            platform = Platform(name=name, url=url, notes=notes)
            db.session.add(platform)
            db.session.commit()
            flash(f"Plataforma {name} criada com sucesso.", "success")
            return redirect(url_for("main.acessos_list"))
    return render_template("acessos/platform_form.html")


@bp.route("/acessos/plataforma/<int:platform_id>/excluir", methods=["POST"])
@roles_required("admin")
def platform_delete(platform_id: int):
    from app.models import Platform
    platform = Platform.query.get_or_404(platform_id)
    name = platform.name
    db.session.delete(platform)
    db.session.commit()
    flash(f"Plataforma {name} removida.", "warning")
    return redirect(url_for("main.acessos_list"))


@bp.route("/acessos/conta/nova", methods=["GET", "POST"])
@roles_required("admin")
def account_create():
    ...
    if request.method == "POST":
        ...
        password = request.form.get("password", "").strip()
        max_slots = int(request.form.get("max_slots", 1) or 1)
        notes = request.form.get("notes", "").strip() or None
        if not platform_id or not login or not password:
            flash("Preencha plataforma, login e senha.", "danger")
        else:
            ...
            return redirect(url_for("main.acessos_list"))
    return render_template("acessos/account_form.html", platforms=platforms)


@bp.route("/acessos/conta/<int:account_id>/editar", methods=["GET", "POST"])
@roles_required("admin")
def account_edit(account_id: int):
    ...
    platforms = Platform.query.order_by(Platform.name.asc()).all()
    if request.method == "POST":
        previous_max_slots = account.max_slots
        requested_max_slots = int(request.form.get("max_slots", account.max_slots) or account.max_slots)
        removable_slots = [slot for slot in account.slots if slot.slot_number > requested_max_slots]
        occupied_removals = [slot for slot in removable_slots if slot.client_id is not None]

        if requested_max_slots < 1:
            flash("O numero de telas deve ser pelo menos 1.", "danger")
            return render_template("acessos/account_form.html", platforms=platforms, account=account)
        if occupied_removals:
            flash("Desvincule as telas ocupadas acima do novo limite antes de reduzir a conta.", "warning")
            return render_template("acessos/account_form.html", platforms=platforms, account=account)

        account.platform_id = int(request.form.get("platform_id", account.platform_id))
        account.login = request.form.get("login", "").strip() or account.login
        account.password = request.form.get("password", "").strip() or account.password
        account.max_slots = requested_max_slots
        account.notes = request.form.get("notes", "").strip() or None
        if account.max_slots > previous_max_slots:
            from app.models import AccountSlot
        ...
        elif account.max_slots < previous_max_slots:
...
@bp.route("/acessos/conta/<int:account_id>/excluir", methods=["POST"])
@roles_required("admin")
...
@bp.route("/acessos/slot/<int:slot_id>/vincular", methods=["POST"])
@login_required
@roles_required("admin")
def slot_link(slot_id: int):
    slot = AccountSlot.query.get_or_404(slot_id)
    client_id = request.form.get("client_id", "").strip()
    if not client_id:
        flash("Selecione um cliente.", "danger")
        return redirect(url_for("main.acessos_list"))
    slot.client_id = int(client_id)
    slot.notes = request.form.get("notes", "").strip() or None
    db.session.commit()
    flash("Tela vinculada ao cliente.", "success")
    return redirect(request.referrer or url_for("main.acessos_list"))


@bp.route("/acessos/slot/<int:slot_id>/desvincular", methods=["POST"])
@login_required
@roles_required("admin")
def slot_unlink(slot_id: int):
    slot = AccountSlot.query.get_or_404(slot_id)
    slot.client_id = None
    slot.notes = None
    db.session.commit()
    flash("Tela desvinculada.", "warning")
    return redirect(request.referrer or url_for("main.acessos_list"))
