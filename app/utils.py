...
def get_timezone() -> ZoneInfo:
    ...
    if name not in _tz_cache:
        try:
            ...
            _tz_cache[name] = ZoneInfo(DEFAULT_TIMEZONE)
...
def renewal_message(client, charge=None, company_name: str | None = None) -> str:
    ...
    due_str = date_br(due_date) if due_date else "a definir"
    amount_str = currency_br(amount)
    parts = [
        "*Ola querido(a) cliente*,",
        "",
        "*SUA CONTA EXPIRA EM BREVE!*",
        "",
        "Seu plano de *" + amount_str + "* vence em:",
        "*" + due_str + "*",
        "",
        "Seu servico(a) contratado e *" + service_name + "*",
        "",
        "Evite o bloqueio automatico do seu sinal",
        "",
        "Para renovar o seu plano agora, clique no link abaixo:",
        payment_link if payment_link else "(link de pagamento sera gerado em breve)",
        "",
        "*Observacoes:*",
        "",
        "Por favor, nos envie o comprovante de pagamento assim que possivel.",
        "",
        "E sempre um prazer te atender.",
    ]
    return "\n".join(parts)
...
#Rotas que recebem POST externo e nao precisam de CSRF
_CSRF_EXEMPT = {
    "/integracoes/mercadopago/webhook",
}
